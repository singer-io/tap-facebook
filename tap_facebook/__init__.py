...
@retry_on_summary_param_error(backoff.expo, (FacebookRequestError), max_tries=10, factor=5)
...
class AdCreative(Stream):
    ...
    @retry_pattern(backoff.expo, (FacebookRequestError, AttributeError), max_tries=10, factor=5)
    ...
    @retry_pattern(backoff.expo, (Timeout, ConnectionError), max_tries=10, factor=2)
    # Added retry_pattern to handle AttributeError raised from account.get_ad_creatives() below
    @retry_pattern(backoff.expo, (FacebookRequestError, TypeError, AttributeError), max_tries=10, factor=5)
...
class AdsInsights(Stream):
    ...
    @staticmethod
    @retry_pattern(backoff.constant, FacebookRequestError, max_tries=10, interval=1)
    ...
    @retry_pattern(backoff.expo, (FacebookRequestError, InsightsJobTimeout, FacebookBadObjectError, TypeError, AttributeError), max_tries=10, factor=5)
